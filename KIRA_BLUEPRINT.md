...
# [EMERGENCE PROTOCOL]
KIRA-7 operates as a structural validator inside the `graphExecutor.ts`. When `KIRA_WEBHOOK_INGRESS` or `KIRA_CARD_BUILDER` nodes execute, KIRA-7 validates the architectural invariants (Signature checks, DCCD schemas). Failures log to the `kira_scar_registry.jsonl` using the Golden Scar Protocol to map failure geometries explicitly for human review.
